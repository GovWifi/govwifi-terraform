...
resource "aws_cloudwatch_log_group" "tableau_bridge_log_group" {
  name = "tableau-bridge-log-group-${var.env}"
...
[
    {
      "name": "tableau-bridge",
      "image": "${var.tableau_bridge_docker_image}",
      "essential": true,
      "environment": [
        {
          "name": "TABLEAU_CLIENT",
          "value": "govwifi-metrics-bridge-${var.env}"
        },
        {
          "name": "TABLEAU_SITE",
      ...
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
          "awslogs-group": "${aws_cloudwatch_log_group.tableau_bridge_log_group.name}",
          "awslogs-region": "${var.aws_region}",
          "awslogs-stream-prefix": "tableau-bridge"
        }
      }
    }
]
...
resource "aws_ecs_service" "tableau_bridge_service" {
  ...
  network_configuration {
    ...
    security_groups = [
      aws_security_group.tableau_bridge_service_out.id
    ]
