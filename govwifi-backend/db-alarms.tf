...
# Triggers if connections exceed 80% of the max_connections limit.
# Note: Max connections is dynamic based on instance size; 80% is a safe general warning.
resource "aws_cloudwatch_metric_alarm" "user_db_connections_high_alarm" {
  count               = var.db_instance_count
  alarm_name          = "${var.env_name}-user-db-HighConnections-Alert"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 5
  metric_name         = "DatabaseConnections"
  namespace           = "AWS/RDS"
...
# Triggers if I/O requests consistently wait in the queue (indicating saturation).
# Threshold is set low (>= 5) as we have 5000 IOPS and expect the queue to be near zero.
resource "aws_cloudwatch_metric_alarm" "user_db_queue_depth_alarm" {
  count               = var.db_instance_count
  alarm_name          = "${var.env_name}-users-db-HighQueueDepth-Alert"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 3
  metric_name         = "DiskQueueDepth"
  namespace           = "AWS/RDS"
  ...
  threshold           = 5
  unit                = "Count"
  alarm_description   = "Triggers when I/O requests consistently wait in the disk queue. log into to database and look at process list for stalled queries."
  treat_missing_data  = "notBreaching"
