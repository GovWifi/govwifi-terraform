resource "aws_vpc" "smoke_tests" {
  ...
  tags = {
    Name = "Govwifi ${var.env_subdomain} Canary-tests"
  }
}

resource "aws_internet_gateway" "canary_tests" {
  vpc_id = aws_vpc.smoke_tests.id

  tags = {
    Name = "${var.env_subdomain}-canary-tests"
  }
}

resource "aws_nat_gateway" "canary_tests_a" {
  allocation_id = aws_eip.canary_tests_a.id
  subnet_id     = aws_subnet.smoke_tests_public_a.id

  tags = {
    Name = "${var.env_subdomain}-public-canary-tests-a"
  }

  # To ensure proper ordering, it is recommended to add an explicit dependency
  # on the Internet Gateway for the VPC.
  depends_on = [aws_internet_gateway.canary_tests]
}

resource "aws_nat_gateway" "canary_tests_b" {
  allocation_id = aws_eip.canary_tests_b.id
  subnet_id     = aws_subnet.smoke_tests_public_b.id

  tags = {
    Name = "${var.env_subdomain}-public-canary-tests-b"
  }
...
resource "aws_subnet" "smoke_tests_private_a" {
  ...
  tags = {
    Name = "${var.env_subdomain}-canary-tests-private-a"
  }
}
...
resource "aws_subnet" "smoke_tests_private_b" {
  ...
  tags = {
    Name = "${var.env_subdomain}-canary-tests-private-b"
  }
}
...
resource "aws_subnet" "smoke_tests_public_a" {
  ...
  tags = {
    Name = "${var.env_subdomain}-canary-tests-public-a"
  }
}
...
resource "aws_subnet" "smoke_tests_public_b" {
  ...
  tags = {
    Name = "${var.env_subdomain}-canary-tests-public-b"
  }
}



resource "aws_route_table" "canary_tests_public" {
  vpc_id = aws_vpc.smoke_tests.id

  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.canary_tests.id
  }

  tags = {
    Name = "${var.env_subdomain}-public-canary-tests"
  }
}

resource "aws_route_table_association" "smoke_tests_public_a" {
  subnet_id      = aws_subnet.smoke_tests_public_a.id
  route_table_id = aws_route_table.canary_tests_public.id
}
...
resource "aws_route_table" "smoke_tests_private_a" {
  ...
  route {
    cidr_block     = "0.0.0.0/0"
    nat_gateway_id = aws_nat_gateway.canary_tests_a.id
  }
...
resource "aws_route_table" "smoke_tests_private_b" {
  ...
  route {
    cidr_block     = "0.0.0.0/0"
    nat_gateway_id = aws_nat_gateway.canary_tests_b.id
  }
...
resource "aws_eip" "canary_tests_a" {
  vpc = true
  tags = {
    Name = "${var.env_subdomain}-canary-tests-a"
  }
}

resource "aws_eip" "canary_tests_b" {
  vpc = true
  tags = {
    Name = "${var.env_subdomain}-canary-tests-b"
  }
}
