...
resource "aws_cloudwatch_event_target" "publish_organisation_count" {
  target_id = "${var.env_name}-publish-organisation-count"
  arn       = aws_ecs_cluster.admin_cluster.arn
  rule      = aws_cloudwatch_event_rule.daily_publish_organisation_count.name
  role_arn  = aws_iam_role.scheduled_task.arn
  ...
  input = <<EOF
  {
    "containerOverrides": [
      {
        "name": "admin",
        "command": ["bundle", "exec", "rake", "metrics:publish_organisation_count"]
      }
    ]
  }
  EOF
...
resource "aws_cloudwatch_event_target" "publish_orgs_with_no_physical_address_for_ip_count" {
  ...
  input = <<EOF
  {
    "containerOverrides": [
      {
        "name": "admin",
        "command": ["bundle", "exec", "rake", "metrics:publish_orgs_with_no_physical_address_for_ip_count"]
      }
    ]
  }
  EOF
